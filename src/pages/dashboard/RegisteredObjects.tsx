...
import { Package, Search, Trash2, MapPin, Clock, ChevronLeft, ChevronRight, X, AlignLeft, Navigation, Pencil, Save } from 'lucide-react';
...
function ImageGallery({ urls, labels }: { urls: string[]; labels?: string[] }) {
  ...
  return (
    <>
      <div className="relative w-full h-44 rounded-xl overflow-hidden mb-4 group cursor-pointer"
        onClick={() => setLightbox(true)}
      >
        <img src={urls[idx]} alt={labels?.[idx] ?? `photo ${idx + 1}`}
        ...
        <div className="flex gap-1.5 mb-4 overflow-x-auto pb-1">
          {urls.map((url, i) => (
            <img key={url} src={url} alt=""
              onClick={() => setIdx(i)}
              className={`w-10 h-10 object-cover rounded-lg cursor-pointer shrink-0 transition-all ${i === idx ? 'ring-2 ring-blue-500 opacity-100' : 'opacity-60 hover:opacity-90'}`}
            />
          ))}
        </div>
      )}
      ...
      {lightbox && (
        ...
        >
          <button className="absolute top-4 right-4 p-2 bg-white/20 rounded-full text-white hover:bg-white/30"
            onClick={() => setLightbox(false)}
          ><X className="w-5 h-5" /></button>

          <button className="absolute left-4 p-2 bg-white/20 rounded-full text-white hover:bg-white/30 disabled:opacity-20"
            disabled={idx === 0}
          ...
          <div onClick={e => e.stopPropagation()} className="flex flex-col items-center gap-3">
            ...
            />
            {labels?.[idx] && (
              <span className="px-3 py-1 bg-white/20 text-white text-sm rounded-full">{labels[idx]}</span>
            )}
          ...
          <button className="absolute right-4 p-2 bg-white/20 rounded-full text-white hover:bg-white/30 disabled:opacity-20"
        ...
        {filteredObjects.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredObjects.map(obj => {
              ...
              return (
                <div key={obj.id} className="bg-gray-50 dark:bg-gray-700/50 rounded-2xl p-5 hover:shadow-lg transition-shadow">
                  ...
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">{obj.object_name}</h3>
                  ...
                  {obj.description && (
                    <div className="flex items-start gap-2 mb-3 p-2.5 bg-white dark:bg-gray-800 rounded-xl">
                      ...
                      <p className="text-xs text-gray-600 dark:text-gray-400 line-clamp-2">{obj.description}</p>
                    </div>
                  )}
                  ...
                  <div className="space-y-2 mb-4">
                    ...
                    {obj.secondary_location && (
                      <div className="flex items-center gap-2 text-sm text-purple-600 dark:text-purple-400">
                        <MapPin className="w-4 h-4 shrink-0" />
                        <span>Spot: {obj.secondary_location}</span>
                      </div>
                    )}
                  ...
                  <div className="flex flex-col gap-2">
                    {directionsUrl && (
                      ...
                      >
                        ...
                        <span className="text-sm font-medium">Get Directions to {obj.last_known_location}</span>
                      </a>
                    )}
                    ...
                    <button onClick={() => setDeleteId(obj.id)}
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <datalist id="rooms-list">
                    {rooms.map(r => <option key={r.id} value={r.room_name} />)}
                  </datalist>
                </div>
              </div>
